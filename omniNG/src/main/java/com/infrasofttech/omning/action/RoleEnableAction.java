 ...
 *
 */
public class RoleEnableAction extends ActionSupport implements  ServletRequestAware {

	private static final long serialVersionUID = -5539422250920232971L;
	private static final Logger logger = Logger.getLogger(RoleEnableAction.class);
	...
	public String execute() throws Exception {
		try{
			...
			{ 
				System.out.println("RoleEnableAction.execute() SESSION EXPIRED");
				//Session expired 
				...
				System.out.println("RoleEnableAction.execute()");
				...
				if(request.getParameter("roleCode") != null){
					...
					roleMst = (RoleMst) roleMstService.getRoleMstByTenantNRoleNIsActive(tenantId, roleCode);
					if(roleMst!=null){
						roleMst.setIsActive(true);
						roleMstService.saveOrUpdate(roleMst);
					}
					else{
						System.out.println("Error while Enabling Role");
					}
					}
					catch(NumberFormatException e){
						System.out.println("RoleEnableAction CATCH");
						e.printStackTrace();
					}
				}
