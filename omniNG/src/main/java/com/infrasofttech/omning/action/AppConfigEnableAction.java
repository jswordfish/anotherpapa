...
public class AppConfigEnableAction extends ActionSupport implements  ServletRequestAware {

	private static final long serialVersionUID = -5539422250920232971L;
	private static final Logger logger = Logger.getLogger(AppConfigEnableAction.class);
		...
		try{
			...
			{ 
				...
				if(request.getParameter("branchCode") != null){
					...
					appConfig = (AppConfig) appConfigService.getAppConfigByIsActiveFalseNTenantNBranch(tenantId, branchCd);
					if(appConfig!=null){
						appConfig.setIsActive(true);
						appConfigService.saveOrUpdate(appConfig);
					}
					else{
						System.out.println("Error while Enabling appConfig");
					}
					}
					catch(NumberFormatException e){
						System.out.println("AppConfigEnableAction CATCH");
						e.printStackTrace();
					}
				}
