...
public class BranchEditAction extends ActionSupport implements
		/* ModelDriven<BranchMst>, */ServletRequestAware {

	private static final long serialVersionUID = -5539422250920232971L;
	private static final Logger logger = Logger
			.getLogger(BranchEditAction.class);
	...
	List<ModuleMst> moduleSelected = new ArrayList<ModuleMst>();
	...
	public String execute() throws Exception {
		try {
			if (request.getSession(false) == null) {
				...
				String updateBranchCode = request.getParameter("id");

				BranchMstService branchMstService = (BranchMstService) SpringUtil
						.getSpringUtil().getService("branchMstService");

				BranchMst entity = branchMstService.getBranchMst(tenantId,
						updateBranchCode);
				// setBranchValues(entity);
				branch = entity;
				moduleSelected = branch.getModules();
				for (Iterator iterator = moduleSelected.iterator(); iterator
						.hasNext();) {
					ModuleMst type = (ModuleMst) iterator.next();
					System.out.println("moduleName>>>>>"+type.getModuleName());
				}
				
				
				request.setAttribute("branch", branch);
				 request.setAttribute("moduleList1",moduleSelected );
	...
	private void setLookupValues() {
		...
		VillageMstService villageMstService = (VillageMstService) SpringUtil
				.getSpringUtil().getService("villageMstService");
			List<VillageMst> villageList = villageMstService
	...
	public List<ModuleMst> getModuleSelected() {
		return moduleSelected;
	}


	public void setModuleSelected(List<ModuleMst> moduleSelected) {
