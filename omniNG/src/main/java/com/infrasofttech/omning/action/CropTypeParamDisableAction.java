...
public class CropTypeParamDisableAction extends ActionSupport implements  ServletRequestAware {

	private static final long serialVersionUID = -5539422250920232971L;
	private static final Logger logger = Logger.getLogger(CropTypeParamDisableAction.class);
	...
	public String execute() throws Exception {
		try{
			...
			{ 
				...
				System.out.println("CropTypeParamDisableAction.execute()");
				...
				if(request.getParameter("cropType") != null){
					try{
						...
						cropTypeParam = (CropTypeParam) cropTypeParamService.getUniqueCropTypeParam(tenantId, cropType, fromDate);
					if(cropTypeParam!=null){
						cropTypeParam.setIsActive(false);
						cropTypeParamService.saveOrUpdate(cropTypeParam);
					}
					else{
						System.out.println("error while disbling CropType");
					}
					}
					catch(NumberFormatException e){
						System.out.println("CropTypeDisableAction CATCH");
						e.printStackTrace();
					}
				}
