 ...
 *
 */
public class BranchEditUpdateAction extends ActionSupport implements ServletRequestAware {
	...
	private BranchMst setValuesToBranchMst(BranchMst branch){
		...
		branch.setLastModifiedDate(lastModifiedDate);
		branch.setLastModifiedBy(loginId);
...
//		branch.setAuthStatus(OmniConstants.AUTH_AUTHORIZED);
		...
		if(mods!=null){
			...
			for(;i<mods.length; i++){
				...
				moduleMst.setModuleCode(mods[i].toString());
