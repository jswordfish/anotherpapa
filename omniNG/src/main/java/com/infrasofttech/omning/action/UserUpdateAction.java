...
public class UserUpdateAction extends ActionSupport implements ModelDriven<UserMst>, ServletRequestAware {
	...
	public String execute() throws Exception {
		try{
			...
			{ 
				...
				UserMst userMst = userMstService.getUserBasedOnLogin(tenantId, loginId1);
				if (userMst != null) {
				System.out.println("User is not null!!!");
				
				UserMst entity = getUserMstObject(userMst);
				
				//UserMstService userMstService = (UserMstService) SpringUtil.getSpringUtil().getService("userMstService");

				entity = userMstService.saveOrUpdate(entity);
				System.out.println("password:"+entity.getPwd());
	...
	private UserMst getUserMstObject(UserMst entity){
		...
		String sal[] = request.getParameterValues("userSalutation");
		if(sal != null && sal.length > 0){
			entity.setUserSalutation(sal[0].toString());
		}
		...
		if(type != null && type.length > 0){
			entity.setUserTypeCode(type[0].toString());
		}
