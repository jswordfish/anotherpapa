...
<head>
	...
	<script>
	function addProduct(){
		document.forms["productForm"].action = "productCRUD.action";
		document.forms["productForm"].submit();
	}
	
	function productViewDisable(){
		document.forms["productForm"].action = "productViewDisable.action";
		document.forms["productForm"].submit();
	}	
...
<br>
	...
	<form name="productForm" >
		...
		<button type="button" class="btn btn-primary" onClick="addProduct();"><s:text name="global.btn.add.product"/></button>
		<% } %>
		&nbsp;
		<button type="button" class="btn btn-primary" onClick="productViewDisable();"><s:text name="global.lbl.disabled.products"/></button>
	</form>
	
    <table class="table">
        <thead>
            <tr>				
				<th><s:text name="global.val.product.code" /></th>
				<th><s:text name="global.val.product.name" /></th>
				<th><s:text name="global.val.module.type" /></th>
				<th><s:text name="global.val.account.open.level" /></th>
				<th><s:text name="global.lbl.voucher.entry.level" /></th>
...
if(null != roleMenuActionAccess)
				...
				<a href="productCRUD.action?productCode=<%= product.getProductCode() %>&moduleCode=<%= product.getModuleCode() %>&mode=UPDATE"><s:text name="global.lbl.edit" /></a>   
					<% } %> 
					<%
  if(null != roleMenuActionAccess)
   				if(roleMenuActionAccess.getDisableAction().getCode().equals(MENUACTIONSTATUS.ACTIVE.getCode())) {%> 	
					<a href="productDisable.action?productCode=<%= product.getProductCode() %>&moduleCode=<%= product.getModuleCode() %>&mode=DISABLE"><s:text name="global.lbl.disable" /></a>  
				<% } %>
				</td>
            </tr>
            <%
