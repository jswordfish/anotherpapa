...
<!--     <style class="cp-pen-styles">label{margin-left: 20px;}
	...
	$(document).ready(
			...
			var data = [];
				...
				<c:forEach  var="branches" items="${moduleSelected}">
				
				data.push('<c:out value="${branches.moduleCode}"/>');
				</c:forEach>
				for (var i = 0; i < data.length; i++) {
					$('#idModule option').map(function() {
						if ($(this).val() == data[i])
							return this;
...
<%--   %> --%>
<body>
<form id="omniBranchEditForm" name="topForm" method="POST" action="branchEditUpdate">
<%
...
<tr class="active">
	...
	<td class="col-md-1"><input readonly id="branchCode" name="branchCode" type="text"  value="<%= branch.getBranchCode() %>"/></td>
	<td class="col-md-1"><s:text name="global.lbl.branch.ref.code" /></td>
	<td class="col-md-1"><input id="branchRefCode" name="branchRefCode" type="text" value="<%= branch.getBranchRefCode() %>" /></td>
</tr>
<tr class="active">
	<td class="col-md-1"><s:text name="global.lbl.branch.name" /></td>
	<td class="col-md-1"><input id="branchName" name="branchName" type="text"  value="<%= branch.getBranchName() %>"/></td>
	...
	<td class="col-md-1">
		<select id="branchType" name="branchType">
			<% for(Map.Entry<String, String> entry : mapBranchType.entrySet()) { 
					if(null != branch.getBranchTypeCode()){
						if(branch.getBranchTypeCode().equalsIgnoreCase(entry.getKey())) { %>
	...
	<td class="col-md-1">
		<select id="village" name="village">
			<% for(Map.Entry<String, String> entry : villages.entrySet()) { 
					if(null!=branch.getVillageCode() &&
							branch.getVillageCode().equalsIgnoreCase(entry.getKey())) { 
			%>
				<option value="<%=entry.getKey()%>" SELECTED><%=entry.getValue() %></option>
			<%		} else { %>
				<option value="<%=entry.getKey() %>"><%=entry.getValue() %></option>
	...
	<td class="col-md-1">
		<select id="district" name="district">
			<% for(Map.Entry<String, String> entry : districts.entrySet()) { 
					if(null != branch.getDistrictCode() &&
							branch.getDistrictCode().equalsIgnoreCase(entry.getKey())) { 
			%>
				<option value="<%=entry.getKey()%>" SELECTED><%=entry.getValue() %></option>
			<%		} else { %>
				<option value="<%=entry.getKey() %>"><%=entry.getValue() %></option>
	...
	<td class="col-md-1">
		<select id="state" name="state">
			<% for(Map.Entry<String, String> entry : states.entrySet()) { 
					if(null != branch.getStateCode() &&
							branch.getStateCode().equalsIgnoreCase(entry.getKey())) { 
			%>
				<option value="<%=entry.getKey()%>" SELECTED><%=entry.getValue() %></option>
			<%		} else { %>
				<option value="<%=entry.getKey() %>"><%=entry.getValue() %></option>
	...
	<td class="col-md-1">
		<select id="country" name="country">
			<% for(Map.Entry<String, String> entry : countries.entrySet()) { 
					if(null != branch.getCountryCode() && 
							branch.getCountryCode().equalsIgnoreCase(entry.getKey())) { 
			%>
				<option value="<%=entry.getKey()%>" SELECTED><%=entry.getValue() %></option>
			<%		} else { %>
				<option value="<%=entry.getKey() %>"><%=entry.getValue() %></option>
...
<tr class="active">
	...
	<td class="col-md-1">
		<input id="tele1" name="tele1" type="text" value="<%= branch.getTele1() %>" />
		<input id="tele2" name="tele2" type="text" value="<%= branch.getTele2() %>" />
	</td>
...
<tr class="active">
	...
	<td class="col-md-1">
		<select id="fromDay" name="fromDay">
			<% for(Map.Entry<String, String> entry : mapDays.entrySet()) { 
					if(null != branch.getBranchDayFromCode() && 
							branch.getBranchDayFromCode().equalsIgnoreCase(entry.getKey())) { 
			%>
				<option value="<%=entry.getKey()%>" SELECTED><%=entry.getValue() %></option>
			<%		} else { %>
				<option value="<%=entry.getKey() %>"><%=entry.getValue() %></option>
	...
	<td class="col-md-1">
		<select id="toDay" name="toDay">
			<% for(Map.Entry<String, String> entry : mapDays.entrySet()) { 
					if(null != branch.getBranchDayToCode() && 
							branch.getBranchDayToCode().equalsIgnoreCase(entry.getKey())) { 
			%>
				<option value="<%=entry.getKey()%>" SELECTED><%=entry.getValue() %></option>
			<%		} else { %>
				<option value="<%=entry.getKey() %>"><%=entry.getValue() %></option>
...
<tr class="active">
	...
	<td class="col-md-1">
		<select id="finMonthStart" name="finMonthStart">
			<% for(Map.Entry<String, String> entry : mapMonths.entrySet()) { 
					if(null != branch.getFinYearFromCode() &&
							branch.getFinYearFromCode().equalsIgnoreCase(entry.getKey())) { 
			%>
				<option value="<%=entry.getKey()%>" SELECTED><%=entry.getValue() %></option>
			<%		} else { %>
				<option value="<%=entry.getKey() %>"><%=entry.getValue() %></option>
	...
	<td class="col-md-1">
		<select id="finMonthEnd" name="finMonthEnd">
			<% for(Map.Entry<String, String> entry : mapMonths.entrySet()) { 
					if(null != branch.getFinYearToCode() &&
							branch.getFinYearToCode().equalsIgnoreCase(entry.getKey())) { 
			%>
				<option value="<%=entry.getKey()%>" SELECTED><%=entry.getValue() %></option>
			<%		} else { %>
				<option value="<%=entry.getKey() %>"><%=entry.getValue() %></option>
...
<tr class="active">
	...
	<td class="col-md-1">
		<select id="weeklyOff1" name="weeklyOff1">
			<% for(Map.Entry<String, String> entry : mapDays.entrySet()) { 
					if(null != branch.getWeeklyOff1Code() &&
							branch.getWeeklyOff1Code().equalsIgnoreCase(entry.getKey())) { 
			%>
				<option value="<%=entry.getKey()%>" SELECTED><%=entry.getValue() %></option>
			<%		} else { %>
				<option value="<%=entry.getKey() %>"><%=entry.getValue() %></option>
	...
	<td class="col-md-1">
		<select id="weeklyOff2" name="weeklyOff2">
			<% for(Map.Entry<String, String> entry : mapDays.entrySet()) { 
					if(null != branch.getWeeklyOff2Code() &&
							branch.getWeeklyOff2Code().equalsIgnoreCase(entry.getKey())) { 
			%>
						<option value="<%=entry.getKey()%>" SELECTED><%=entry.getValue() %></option>
			<%		} else { %>
						<option value="<%=entry.getKey() %>"><%=entry.getValue() %></option>
...
<tr class="active">
	...
	<td class="col-md-1">
		<select id="halfOff1" name="halfOff1">
			<% for(Map.Entry<String, String> entry : mapDays.entrySet()) { 
					if(null != branch.getWeeklyHalfOff1Code() &&
							branch.getWeeklyHalfOff1Code().equalsIgnoreCase(entry.getKey())) { 
			%>
				<option value="<%=entry.getKey()%>" SELECTED><%= entry.getValue() %></option>
			<%		} else { %>
				<option value="<%= entry.getKey() %>"><%=entry.getValue() %></option>
	...
	<td rowspan="3" class="col-md-1">
		<select id="idModule" name="moduleList" multiple="multiple" size="4">
...
<tr class="active">
	...
	<td class="col-md-1">
		<select id="halfOff2" name="halfOff2">
			<% for(Map.Entry<String, String> entry : mapDays.entrySet()) { 
					if(null != branch.getWeeklyHalfOff2Code() &&
							branch.getWeeklyHalfOff2Code().equalsIgnoreCase(entry.getKey())) { 
			%>
				<option value="<%=entry.getKey()%>" SELECTED><%=entry.getValue() %></option>
			<%		} else { %>
				<option value="<%=entry.getKey() %>"><%=entry.getValue() %></option>
...
<tr class="active">
	...
	<%
		Boolean flag=true;
		String[] arg = branch.getNonWorkingHalfOffs().split(",");
	
		for(Integer i=1 ; i <= 5 ; i++){
			for(int j=0; j < arg.length ; j++){
				if(arg[j].toString().equalsIgnoreCase(i.toString()))
				{
					flag = true;
					%>
					<input type="checkbox" id="nwhf" name="nwhf" value="<%= i %>" checked>&nbsp;<%= i %>&nbsp;
					<%
					break;
				}else{
					flag=false;
			...
			if(flag==false){
				//show checkbox UNCHEKCED
				%>
				<input type="checkbox" id="nwhf" name="nwhf" value="<%= i %>">&nbsp;<%= i %>&nbsp;
				<%
			}
		}  // END OF FOR (i<5) LOOP
	%>			
