...
<script src='http://codepen.io/assets/editor/live/css_live_reload_init.js'></script>
	...
	<script>
	function callAdd(){
		document.forms["omniForm"].action = "cropTypeCRUD.action";
		document.forms["omniForm"].submit();
	}
	function callViewDisabled(){
		document.forms["omniForm"].action = "cropTypeDisableList.action";
		document.forms["omniForm"].submit();
	}	
	</script>
...
<br>
	...
	if(roleMenuActionAccess.getCreateAction().getCode().equals(MENUACTIONSTATUS.ACTIVE.getCode())) {%> 
		<button type="button" class="btn btn-primary" onClick="callAdd();"><s:text name="global.btn.crop.type"/></button>
		<%} %>
		&nbsp;
		<button type="button" class="btn btn-primary" onClick="callViewDisabled();"><s:text name="global.btn.disabled.crop.type"/></s></button>
	</form>
    ...
    <table class="table">
		...
		List<CropTypeParam> cropTypeList = (List<CropTypeParam>) request.getAttribute("cropTypes");
        ...
        <tbody>
        	...
        	<tr class="active">
				...
				<a href="cropTypeCRUD.action?cropType=<%= cropType.getCropType() %>&fromDate=<%= cropType.getFromDate()%>&mode=UPDATE"><s:text name="global.ref.edit"></s:text></a>
				<% } %>
					<% if(null != roleMenuActionAccess)
  					 if(roleMenuActionAccess.getDisableAction().getCode().equals(MENUACTIONSTATUS.ACTIVE.getCode())) {%> 				   
					<a href="cropTypeDisable.action?cropType=<%= cropType.getCropType() %>&fromDate=<%= cropType.getFromDate() %>&mode=DISABLE"><s:text name="global.ref.disable"></s:text></a> 
					<% } %> 
				</td>
            </tr>
            <%
