...
<script src='http://codepen.io/assets/editor/live/css_live_reload_init.js'></script>
	...
	<script>
	function callAdd(){
		document.forms["omniForm"].action = "GLMstCRUD.action";
		document.forms["omniForm"].submit();
	}
	function callViewDisabled(){
		document.forms["omniForm"].action = "GLMstDisableList.action";
		document.forms["omniForm"].submit();
	}	
	</script>
...
<br>
	...
	<form name="omniForm" method="POST">
		...
		<input type="hidden" name="fyCode" value="-1">
...
if(null != roleMenuActionAccess)
	if(roleMenuActionAccess.getCreateAction().getCode().equals(MENUACTIONSTATUS.ACTIVE.getCode())) {%> 
		<button type="button" class="btn btn-primary" onClick="callAdd();"><s:text name="global.btn.create.gl" /></button>
		<%} %>
		&nbsp;
		<button type="button" class="btn btn-primary" onClick="callViewDisabled();"><s:text name="global.btn.show.disabled.gl" /></button>
	</form>
    ...
    <table class="table">
        ...
        <tbody>
        	...
        	<tr class="active">
				...
				<a href="GLMstCRUD.action?glCode=<%= glMst.getGlCode() %>&fyCode=<%= glMst.getFyCode() %>&mode=UPDATE"><s:text name="global.ref.edit" /></a>
				<% } %>
					<%
    if(null != roleMenuActionAccess)
   if(roleMenuActionAccess.getDisableAction().getCode().equals(MENUACTIONSTATUS.ACTIVE.getCode())) {%> 				   
					<a href="GLMstDisable.action?glCode=<%= glMst.getGlCode() %>&fyCode=<%= glMst.getFyCode() %>&mode=DISABLE"><s:text name="global.ref.disable" /></a> 
					<% } %> 
				</td>
            </tr>
            <%
